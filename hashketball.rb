...
def game_hash
  game_hash = {
      home:{team_name:"Brooklyn Nets", colors:["Black", "White"] ,
        players:[  #needs to be an array of hashes given the bottom level
            {
...
def players #method that pulls all players hashes {name & data}
  home_players = game_hash.fetch(:home).fetch(:players) #fetch returns value for hash key that matches
  away_players = game_hash.fetch(:away).fetch(:players)
  home_players + away_players
end

def find_the_player(name) #method that looks for the first match
...
def num_points_scored(name)
  player = find_the_player(name) #define player as the hash that meets the name criteria
...
def player_stats(player)
  find_the_player(player).reject{|name_key| name_key==:player_name} #reject returns all values that are false against the blocks
end
